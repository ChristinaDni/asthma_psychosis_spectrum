...
data<- read.table("topregions_asthma_schizophrenia.txt", sep="\t", header=T)

ID<- data %>% split(x=data, f=data$SNP)

#Read in the GWAS data to extract the regions
gwas_data<- read_table("/dir/data_repo/clean/neuropsychiatric/B37:SCHIZOPHRENIA:2022:EUR.txt")
...
setwd("/dir/revision_03_07_25/pwcoco/trait/data/schizophrenia/trait2")

file_out_snp<- paste0("scz","_", snp, ".txt")
print(file_out_snp)
...
setwd("/dir/revision_03_07_25/pwcoco/trait/data")
data<- read.table("topregions_scz_asthma.txt", sep="\t", header=T)
