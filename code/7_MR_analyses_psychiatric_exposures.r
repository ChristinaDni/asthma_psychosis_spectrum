...
outcome_files <- "/dir/data_repo/clean/respiratory/B37:ASTHMA:2022:EUR.txt"

exposure_files <- list.files(path="/dir/data_repo/clean/neuropsychiatric", 
pattern="*.txt", full.names=TRUE, recursive=FALSE) 

exposure_files<- exposure_files[c(6,19)]


library(TwoSampleMR)
library(readr)

lapply(list(exposure_files), function(i){
...
for (var in iterations){
  ...
  exposure<- exposure_file[which(exposure_file$SNP%in%merged$SNP),]
	...
	i<- format_data(i, type = "exposure", snps = NULL, header = TRUE, 
                ...
                other_allele_col = "A2", pval_col = "P", id_col="pheno", samplesize_col="NTOT", phenotype_col="pheno", eaf_col="FRQ",
...
if(file.exists("instruments_psychiatric.txt") == TRUE) {
  write.table(exposure_clumped,"instruments_psychiatric.txt", sep="\t", quote=F, row.names=F, col.names=F, append=TRUE)
} else { 
  write.table(exposure_clumped, "instruments_psychiatric.txt", sep="\t", quote=F, row.names=F, append=FALSE)
}
...
setwd("/dir/revision_03_07_25/mr/results")
if(file.exists("harmonised_reverse.txt") == TRUE) {
  write.table(dat,"harmonised_reverse.txt", sep="\t", quote=F, row.names=F, col.names=F, append=TRUE)
} else { 
  write.table(dat, "harmonised_reverse.txt", sep="\t", quote=F, row.names=F, append=FALSE)
}
...
setwd("/dir/revision_03_07_25/mr/results")
if(file.exists("reverse_results_asthma_psychoses.txt") == TRUE) {
  write.table(or,"reverse_results_asthma_psychoses.txt", sep="\t", quote=F, row.names=F, col.names=F, append=TRUE)
} else { 
  write.table(or, "reverse_results_asthma_psychoses.txt", sep="\t", quote=F, row.names=F, append=FALSE)
}
